    ...
    pockets.forEach(pocket => {
        ...
        card.innerHTML = `
                    ...
                    <div class="flex items-center gap-3.5">
                        ...
                        <div class="truncate">
                            <h3 class="font-bold text-slate-900 dark:text-white text-lg truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                ${escapeHtml(pocket.name)}
                            </h3>
                            <span class="text-xs text-slate-400 dark:text-slate-500">Criada em ${formatDate(pocket.created_at.split('T')[0])}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-1 shrink-0">
                        <button onclick="viewPocketHistory(${pocket.id}, ${escapeJsString(pocket.name)})" title="Ver Histórico da Caixinha"
            ...
            <div class="grid grid-cols-2 gap-2.5 pt-4 border-t border-slate-100 dark:border-slate-800">
                <button onclick="openTransferModal(${pocket.id}, ${escapeJsString(pocket.name)}, 'deposit')"
                ...
                <button onclick="openTransferModal(${pocket.id}, ${escapeJsString(pocket.name)}, 'withdraw')"
    ...
    try {
        ...
        txs.forEach(tx => {
            ...
            tr.innerHTML = `
                ...
                <td class="py-3 px-3 font-medium text-slate-800 dark:text-slate-200">${escapeHtml(tx.description || (isDeposit ? 'Depósito' : 'Resgate'))}</td>
                <td class="py-3 px-3">
